...
const AeroCanvas = forwardRef(function AeroCanvas(
  ...
  useImperativeHandle(ref, () => ({
    ...
    getRecognitionPixels() {
      ...
      cropCtx.drawImage(sourceCanvas, cropX, cropY, cropWidth, cropHeight, 0, 0, cropWidth, cropHeight);
      ...
      targetCtx.drawImage(cropCanvas, 0, 0, cropWidth, cropHeight, drawX, drawY, drawWidth, drawHeight);
      const targetData = targetCtx.getImageData(0, 0, 28, 28).data;

      const grayscale = [];
      for (let index = 0; index < targetData.length; index += 4) {
        const brightness = Math.max(targetData[index], targetData[index + 1], targetData[index + 2]);
        grayscale.push(Number((brightness / 255).toFixed(4)));
      }
