...
import { X, Users, Copy, Check, Lock, Unlock, Crown, UserMinus } from 'lucide-react';
...
function PartyModal({ isOpen, token, currentParty, onClose, onPartyJoined, onPartyLeft }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-[28px] border border-white/10 bg-[#09111a] p-6 text-slate-100 shadow-2xl">
        <div className="mb-5 flex items-center justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-[0.32em] text-slate-500">AeroScript</p>
            <h3 className="mt-2 text-xl font-semibold text-white">
              {currentParty ? 'Party Lobby' : mode === 'create' ? 'Create Party' : 'Join Party'}
            </h3>
          </div>
